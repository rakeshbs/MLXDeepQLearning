...
Everything discovered through empirical runs on Breakout (pixel observations, M4 Max).
...
model across 49 Atari games (DeepMind's original multi-game setup) to normalize
...
**Fix:** `train_start=50_000` — DeepMind's original ratio of 5% buffer fill before
...
A 1M buffer costs ~53 GB — comfortable on 128 GB M4 Max.
...
## 6. Buffer size: 1M matches the original DeepMind setup

DeepMind's 2015 DQN paper used a 1M transition replay buffer for all Atari games.
With uint8 storage (see §4), this is achievable on a 128 GB M4 Max (~53 GB).
...
DeepMind's Ape-X (2018) ran 256 actors on 256 separate CPU machines, each with
dedicated CPU/RAM, connected to a single GPU learner over a network queue.

On a single M4 Max:
- 8 actors: well-matched to 14 CPU cores, leaves headroom for learner
- 12–16 actors: probably the ceiling before CPU context-switching overhead hurts
- 256 actors: RAM fits (~40 GB for processes alone) but 18 processes/core means
...
## 9. The original DeepMind DQN (2015) specs
...
Your M4 Max runs faster per update than their GPU did. The main gap is total frame
...
This mirrors DeepMind's epsilon annealing (1.0 → 0.1 over 1M frames) adapted for the Ape-X multi-actor setup.
...
- On an M4 Max with a full 1M buffer (~47 GB), save and load each take ~8 seconds.
...
**Observation:** With `epsilon_base_decay=0.9999` and `epsilon_base_min=0.05`, epsilon_base decayed from 0.4 → ~0.08 over ~15k episodes. At 0.08, the Ape-X spread formula makes 7 of 8 actors essentially greedy (epsilons < 0.01). Only actor 0 retains any meaningful exploration.
...
1. **Huber loss** — quadratic for |td| ≤ 1, linear beyond:
   ...
   ```
   This is what DeepMind's 2015 paper called "error clipping" — they clipped TD errors to [-1, 1] before squaring, which is mathematically equivalent.
...
- This is standard in the original DeepMind 2015 DQN and was applied to all 49 Atari games
...
- `reward_shaper` clips the accumulated reward (possibly from multiple brick hits) to [-1, 1] — consistent with DeepMind's per-decision-step clipping
...
- Or train from scratch with fixed LR and fixed epsilon (DeepMind style)

### 22c. Decision: train from scratch, no LR decay, no epsilon decay (since revised)

DeepMind's original DQN used fixed lr=1e-4 and fixed epsilon throughout all 50M frames. Both our LR decay and epsilon decay were added empirically to fight symptoms (late-training instability, exploration collapse) that may themselves be artifacts of the small-buffer / short-run conditions.
...
**Conclusion:** DeepMind's reasoning for 1M was correct. Reverted to 1M.

---

## 24. Life-loss as terminal state (single-ball episodes)

DeepMind's original DQN training trick: treat each life loss as `done=True`, so each ball is its own episode. The game resets on every ball drop.
...
## 27. DeepMind's epsilon schedule vs our Ape-X adaptation

**DeepMind DQN (2015) original:**
...
**DeepMind-equivalent translation:**
...
- We use slower decay (0.9995) and lower floor (0.05) — less aggressive than DeepMind

**Key insight:** DeepMind started at epsilon=1.0 and used the first 1M frames as both warmup and exploration. We start at 0.4 (lower) because our train_start=50k means the random warmup is already built-in — we don't need the full 1.0 starting point.
...
**Takeaway:**
- A 50k global warmup is normal for Ape-X style training.
...
- Keep exploration primarily from the Ape-X epsilon spread rather than a long forced-random phase.
...
**Rule:** This matches real Atari Breakout (and DeepMind's ALE): clearing a wall resets the ball to the paddle and the player must FIRE to begin the next wall.
...
**Why 100:**
The Ape-X paper (Horgan et al. 2018) explicitly updates the target network every 100 training batches. This keeps the target tracking the online network closely, limiting the window for overestimation to accumulate. Double DQN reduces overestimation at action *selection*; frequent target updates reduces it at *evaluation*.

---

## 35. Match DeepMind's epsilon schedule: start=1.0, decay to floor in ~250k updates

**DeepMind DQN (2015):** Single actor, linear epsilon 1.0 → 0.1 over 1M frames = ~250k gradient updates, fixed at 0.1 forever after.

**Our previous config:** `epsilon_base=0.4` → avg starting epsilon across 8 actors was only ~12.1%, barely above DeepMind's floor. The entire high-exploration phase was skipped.
...
## 36. target_update_freq=100 is too frequent — use 2500 to match DeepMind
...
**Why the Ape-X "100 batches" doesn't directly translate:**
The Ape-X paper (Horgan et al. 2018) uses 360 actors. With that many actors, 100 training batches still corresponds to many thousands of environment steps. With only 8 actors, 100 gradient updates is a tiny window — far less environment experience than Ape-X's 100 batches.
...
**Why 2500:**
DeepMind DQN used a hard target update every 10,000 frames. With frame_skip=4, that's 2,500 gradient updates. This gives the online network 2,500 steps to learn before the target catches up — enough stability to train on, without so much lag that the target diverges dangerously.
...
## 37. epsilon_base=1.0 start hurts multi-actor training — Ape-X diversity ≠ DeepMind diversity

**Experiment:** Changed epsilon schedule to match DeepMind: `epsilon_base=1.0`, `decay=0.9990`, `floor=0.3`. Result: BestAvg100 only reached **38.0** vs **56.6** with `epsilon_base=0.4/floor=0.2`.
...
3. **DeepMind's 1.0 start was for a single actor — Ape-X doesn't need it**
   DeepMind started at epsilon=1.0 to ensure diverse early exploration from a single actor. With 8 Ape-X actors spread from epsilon=0.4 down to ~0.01, diversity is already baked in via the spread. Starting at 1.0 adds no diversity benefit — it just pollutes the buffer.

**Rule:** With Ape-X multi-actor setup, epsilon diversity comes from the **spread across actors**, not from high starting epsilon. Keep `epsilon_base=0.4` so even the most exploratory actor isn't playing randomly.
...
**Previous setting:** `target_update_freq=2500` (section 36 — derived as DeepMind's 10,000 frames / frame_skip=4).

**Revised to:** `target_update_freq=10_000`

**Why the 2,500 derivation was wrong for our setup:**
DeepMind's 10,000-frame schedule assumed a single actor where every frame corresponds to one env step and roughly one gradient update. In our Ape-X setup with 8 actors, env steps accumulate much faster than gradient updates. Dividing by frame_skip=4 was a frame→update conversion that doesn't apply the same way here.
...
## Hardware reference (128 GB M4 Max)
